...
playbook_file=${ansible_resource_dir}/playbooks/deploy_activemq.yml
...
ansible-playbook --key-file=${key_file} --ssh-extra-args="-o StrictHostKeyChecking=no" -i ${inventory_file} --limit ${module} -e "targets=activemq_${target}" -e "@${config_file}" ${playbook_file}